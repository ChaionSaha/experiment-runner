...
function pathValidator(input, name) {
  ...
  if (input.match(/[^A-z0-9-\s]/i))
    return (
      name +
      " name can only contain alphanumeric characters, dashes, and spaces"
    );
  if (fs.existsSync(path.join(rootPath, input)))
...
function generateAutocompleteSource(choices, defaultChoices, Seperator) {
  ...
  const mappedDefaultChoices =
    defaultChoices.length &&
    defaultChoices.map((x) => {
      return { value: x };
    });
  return async function (input) {
    input = input || "";
    const fuzzyResult = fuzzy
      .filter(input, mappedChoices, { extract: (x) => x.value })
      .map((x) => {
        return { value: x.original.value };
      });
...
const rootPath = path.join(
  __dirname + `./..${isUsedAsSubmodule ? `/..` : ""}/src`
);
if (!fs.existsSync(rootPath)) fs.mkdirSync(rootPath);
const templatesPath = path.join(
  __dirname,
  `./..${isUsedAsSubmodule ? `/..` : ""}/templates`
);
...
(async function () {
  const { default: inqPrompt, Separator } = await import(
  ...
  function selectWebsite() {
    return new Promise((resolve, reject) => {
      ...
      inqPrompt({
        ...
        source: generateAutocompleteSource(
          websites,
          [
            PromptConstants.WEBSITE,
            PromptConstants.TEMPLATE,
            PromptConstants.EXIT,
          ],
          Separator
        ),
      })
        .then((answeredWebsite) => {
          ...
          if (answeredWebsite === PromptConstants.TEMPLATE)
            return reject(createTemplate());
          if (answeredWebsite !== PromptConstants.WEBSITE)
            return resolve((selectedWebsite = answeredWebsite));
          inq
            .prompt([
              {
                type: "input",
                ...
                name: "website",
                validate: () => pathValidator(answeredWebsite, "Website"),
              },
            ])
            .then((answers) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      inqPrompt({
        ...
        source: generateAutocompleteSource(
          campaigns,
          [
            PromptConstants.CAMPAIGN,
            PromptConstants.BACK,
            PromptConstants.EXIT,
          ],
          Separator
        ),
        ...
        .then((answeredCampaign) => {
          ...
          if (answeredCampaign === PromptConstants.BACK)
            return reject(
              selectWebsite()
                .then(selectCampaign)
                .then(selectVariation)
                .then(selectVariationNow)
                .catch(catch$)
            );
          if (answeredCampaign !== PromptConstants.CAMPAIGN)
            return resolve((selectedCampaign = answeredCampaign));
          inq
            .prompt([
              {
                type: "input",
                ...
                name: "campaign",
                validate: () => pathValidator(answeredCampaign, "Campaign"),
              },
            ])
            .then((answers) => {
              selectedCampaign = answers.campaign;
              fs.mkdirSync(
                path.join(rootPath, selectedWebsite, selectedCampaign)
              );
  ...
  function selectVariation() {
    return new Promise((resolve, reject) => {
      const variations = readdirSync_f(
        path.join(rootPath, selectedWebsite, selectedCampaign)
      );
      inqPrompt({
        ...
        .then((answeredVariation) => {
          ...
          if (answeredVariation === PromptConstants.BACK)
            return resolve(
              selectCampaign()
                .then(selectVariation)
                .then(selectVariationNow)
          ...
          if (answeredVariation !== PromptConstants.VARIATION)
            return resolve((selectedVariation = answeredVariation));
          inq
            .prompt([
              {
                type: "input",
                ...
                name: "variation",
                validate: () => pathValidator(answeredVariation, "Variation"),
              },
            ])
            .then((answers) => {
              ...
              inqPrompt({
                ...
                .then((answeredTemplate) => {
                  if (answeredTemplate === PromptConstants.EXIT)
                    return reject();
                  if (answeredTemplate === PromptConstants.BACK)
                    return resolve(
                      selectVariation().then(selectVariationNow).catch(catch$)
                    );
                  ...
                  if (answeredTemplate === PromptConstants.EMPTY) {
                    const experimentCode = `${selectedCampaign.toUpperCase()}-${selectedVariation.toUpperCase()}`;
                    let bodyClass = experimentCode
                      .trim()
                      .replace(/#/g, "-") // Replaces '#' with '-'
                      .replace(/\s+/g, "-") // Replaces spaces with '-'
                      .toLowerCase();

                    const defaultJS = `import waitForElem from "@utils/wait-for-elem";
import waitUntil from "@utils/wait-until";

let experimentCode = '${experimentCode}'; // Add your experiment code here
...
`;
                    const defaultSCSS = `.${bodyClass} {
}`;

                    const indexPath = path.join(variationPath, "index.js");
                    const stylePath = path.join(variationPath, "style.scss");

                    fs.writeFileSync(indexPath, defaultJS);
                    fs.writeFileSync(stylePath, defaultSCSS);
                  } else {
  ...
  function selectVariationNow() {
    readdirSync_f(rootPath).forEach((website) => {
      readdirSync_f(path.join(rootPath, website)).forEach((campaign) => {
        readdirSync_f(path.join(rootPath, website, campaign)).forEach(
          (variation) => {
            const isActiveNow = fs.existsSync(
              path.join(rootPath, website, campaign, variation, ".now")
            );
            if (isActiveNow)
              fs.unlinkSync(
                path.join(rootPath, website, campaign, variation, ".now")
    ...
    logTimer = setTimeout(
      () =>
        console.log(
          ">>> Selected variation: " +
            [selectedWebsite, selectedCampaign, selectedVariation].join(" > ")
        ),
      1e3
    );
  }
      ...
      .prompt([
        {
          ...
          name: "template",
          validate: (input) => pathValidator(input, "Template"),
        },
      ])
      .then((answers) => {
  ...
  if (!directToCampaign && !directToVariation)
  ...
  if (!activeVariation)
    return console.log(
      "No active variation found! Try `npm run select` first."
    );
  ...
  if (directToVariation)
    return selectVariation().then(selectVariationNow).catch(catch$);
})();
