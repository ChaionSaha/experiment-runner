...
const cssFormat =
  (process.env.MINIFY_CSS || "false") === "true" ? "compressed" : "expanded";
    ...
    entries: [
      {
        find: /@utils\/(.*)/,
        replacement: `./../../../../${
          isUsedAsSubmodule ? "experiment-runner/" : ""
        }utils/$1`,
      },
    ],
  }),
...
if (!activeVariation)
  return console.log(
    color.red(
      `No active variation found!\n${color.yellow(
        `Please select a variation first using ${color.red(
          "`npm run select`"
        )}.`
      )}`
    )
...
const variationDir = path.join(
  rootPath,
  activeVariation.website,
  activeVariation.campaign,
  activeVariation.variation
);
if (!fs.existsSync(path.join(variationDir, "dist")))
  fs.mkdirSync(path.join(variationDir, "dist"));
if (!fs.existsSync(path.join(variationDir, "index.js")))
  fs.writeFileSync(path.join(variationDir, "index.js"), "");
if (!fs.existsSync(path.join(variationDir, "style.scss")))
  fs.writeFileSync(path.join(variationDir, "style.scss"), "");

function compileCss(variationDir) {
  try {
    const result = sass.compile(path.join(variationDir, "style.scss"), {
      style: cssFormat,
    });
    ...
    generateEs5Code &&
      (async () => {
        try {
          ...
          const bundledJs = output[0].code;
          fs.writeFileSync(
            path.join(variationPath, "dist", "index.es5.js"),
            bundledJs
          );
        } catch (e) {
...
if (buildOnly)
  return buildToDist(variationDir, true).then(async () => {
    console.log(
      color.green(
        `Build completed successfully & copied @ ${color.italic(
          `${activeVariation.website} > ${activeVariation.campaign} > ${activeVariation.variation}`
        )}`
      )
    );
    const cssToCopy = fs
      .readFileSync(path.join(variationDir, "dist", "style.css"))
      .toString();
    clipboard.writeSync(cssToCopy);
    await (() => new Promise((r) => setTimeout(r, 1e3)))();
    const jsToCopy = fs
      .readFileSync(path.join(variationDir, "dist", "index.js"))
      .toString();
...
app.get("/experiment-runner.js", (req, res) => {
  const bundledJs = fs
    .readFileSync(path.join(__dirname, "ops", "bundle.js"))
    .toString()
  ...
  const website = req.query.origin;
  const configFile = fs
    .readFileSync(path.join(rootPath, activeVariation.website, "config.js"))
    .toString();

  const configRegex = extractRegex(configFile);
  const isMatch = configRegex.test(website);
  if (isMatch) res.send(bundledJs).end();

  res.end();
});
app.get("/experiment-test.js", (req, res) => {
  const bundledJs = fs
    .readFileSync(path.join(__dirname, "ops", "bundle-no-socket.js"))
    .toString()
    .replace(/{{server_port}}/g, serverport)
    .replace(/{{hostname}}/g, req.hostname)
    .replace(/{{protocol}}/g, protocol);
...
buildToDist(variationDir).then(() => {
  if (protocol === "http")
    app.listen(serverport, () =>
      console.log(
        color.green(
          `Server is running at ${color.underline(
    ...
    try {
      ...
      console.error(
        "Error reading key.pem or cert.pem file, please make sure they are available in the root directory"
      );
    }
  }
});
...
const jsWatcher = rollup.watch({
  ...
  output: {
    file: path.join(variationDir, "dist", "index.js"),
    format: buildFormat,
    strict: false,
  },
  plugins: pluginConfigs,
...
wss.on("connection", (ws) => {
  ...
  connected = true;
  console.log(
    colorLog()("Live reloading connected. Watching files for changes...")
  );
});
...
fs.watch(scssPath, (event, filename) => {
  ...
  timer = setTimeout(() => {
    ...
    console.log(
      colorLog()(
        `File ${filename} has been changed at ${new Date().toLocaleTimeString()}, updating...`
      )
    );
...
jsWatcher.on("event", (event) => {
  ...
  if (connected) {
    wss.clients.forEach((tab) =>
      tab.send(JSON.stringify({ event: "change", filename: changedFilePath }))
    );
  } else {
    const textToCopy = fs
      .readFileSync(path.join(variationDir, "dist", "index.js"))
  ...
  console.log(
    colorLog()(
      `File ${changedFilePath} has been changed at ${new Date().toLocaleTimeString()}, ${
        connected ? "reloading.." : "copied to clipboard"
      }.`
    )
  );
});
...
console.log(
  `> Server port: \t\t ${color.italic(
    serverport
  )} \t\t|\t> WebSocket port: \t ${color.italic(wsport)}`
);
console.log(
  `> Protocol: \t\t ${color.italic(
    protocol
  )} \t\t|\t> Copy to clipboard: \t ${color.italic(toCopyToClipboard)}`
);
console.log(
  `> Build format: \t ${color.italic(
    buildFormat
  )} \t\t|\t> Active variation: \t ${color.italic(
    `${activeVariation.website} > ${activeVariation.campaign} > ${activeVariation.variation}`
  )}`
);
console.log(
...
function extractRegex(str = "") {
  const result = str.split("=")[1].trim().split(";")[0].trim();
  const regexStr = result.slice(1, result.lastIndexOf("/"));
  const regexFlag = result.slice(result.lastIndexOf("/") + 1, result.length);
  const newRegex = new RegExp(regexStr, regexFlag);
  return newRegex;
